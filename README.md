...
一个基于官方 Mermaid 渲染库的浏览器本地 Mermaid 在线编辑器。支持实时预览、模板库、多语言界面、语法辅助、主题与布局配置、Markdown 导入导出，以及 SVG / PNG / JPG 图片导出。
...
- 界面支持简体中文、繁体中文（香港）、英语、日语、韩语和俄语；语言选择保存在浏览器 localStorage。
...
- 界面语言同样保存在当前浏览器的 localStorage 中，不会上传到服务器。
